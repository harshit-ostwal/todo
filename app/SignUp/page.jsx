    ...
    return (
        <>
            <div className="bg-zinc-800 text-zinc-950 h-screen w-full font-VisbyRoundBold flex flex-col items-center justify-center overflow-hidden">
                <h1 className="text-amber-100 text-2xl md:text-4xl">SignUp Into Your Account?</h1>
                <div className="w-[95%] md:w-[50%] lg:w-[30%] text-center mt-20">
                    <form className="flex flex-col gap-2">
                        <Input onChange={(e) => setName(e.target.value)} className="rounded-[10px] h-14 text-base" type="text" placeholder="Name" />
                        ...
                        <button className="bg-amber-200 hover:bg-white mt-8 transition-colors duration-300 py-5 rounded-[20px] flex items-center justify-center gap-2" type="submit">Sign Up<LogIn size={24} /></button>
                    </form >
                    ...
                    <Link href={"/SignIn"} className="text-zinc-400 underline">Already User Registered? SignIn</Link>
                </div>
            </div>
        </>
    )
